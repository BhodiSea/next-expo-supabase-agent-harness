...
test('0.5.0 closed EIGHT escapes; seven still stand and the eighth is a recorded extension', () => {
  ...
  // Kept, not deleted. Dropping the wrappers would take the fleet from 22 to 14 against
...
test('the SHIPPED 0.7.0 rampExpiry record equals what the shipped call sites compute', () => {
  ...
  // TWENTY-TWO since 1.0.0 added FOUR, all at minVersion 1.0.0 due 1.1.0 — the injected
  ...
  // behavior-keyed widening of the vertical-anatomy DAL laws, and the census
  // module-name closure over the shipped tools/modules.json (the last two do NOT
  // widen the set — boundaries already carries its 0.9.5 anatomy ramp at this vintage).
  const fresh = classifyForInstall('0.6.0', '0.7.0', sites)
  assert.equal(fresh.expired.length, 0)
  assert.equal(fresh.noting.length, 22)
  assert.deepEqual(
