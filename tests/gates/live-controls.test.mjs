...
import {
	liveControls,
	singleSurfaceGates,
} from "../../template/base/tools/lib/live-controls.mjs";

const SHIPPED_WORKFLOWS = fileURLToPath(
	new URL("../../template/base/github/workflows", import.meta.url),
);
const SHIPPED_TOOLS = fileURLToPath(
	new URL("../../template/base/tools", import.meta.url),
);
...
`,
	...
	assert.equal(
		conditional.has("always"),
		false,
		"an unconditional job must not be marked",
	);
});
...
`,
	...
	assert.ok(
		conditional.has("check-web-e2e.mjs"),
		"it did not run when its only lane skipped",
	);
	assert.ok(live.has("check-secrets.mjs"));
	assert.equal(
		conditional.has("check-secrets.mjs"),
		false,
		"an unconditional lane runs it",
	);
});
...
`,
	...
	assert.equal(
		workflows,
		2,
		"ALL workflows are read — the 0.3.0 correction, applied here",
	);
...
test("the SHIPPED workflows: nine of them, and the path-filtered lanes are marked", () => {
	...
	for (const lane of ["web-e2e", "perf-lane", "mobile-e2e"]) {
		assert.ok(
			conditional.has(lane),
			`${lane} is path-filtered and must be marked conditional`,
		);
	}
...
test("singleSurfaceGates keys by the STEP name, not the filename", () => {
	...
	assert.ok(
		gates.length >= 8,
		`the harness ships more single-surface gates: ${gates.length}`,
	);
});
...
test("THE STEP FOLD does not hide a one-surface step that happens to run two scripts", () => {
	...
	assert.equal(
		folded.length,
		2,
		"both scripts stay reported under the shared step key",
	);
