...
for (const [ip, meta] of Object.entries(manifest.files ?? {})) {
  ...
  if (current !== meta.sha256) {
    into.push(`${ip}: sha256 mismatch against ${MANIFEST} (tampered or hand-edited — or an update did not complete: re-run \`update\`, or \`update --rollback\` to restore the pre-update tree)`)
  }
}
