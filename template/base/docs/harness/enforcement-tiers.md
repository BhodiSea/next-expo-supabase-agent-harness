...
| `check-observability.mjs` | the observability seam (vendor telemetry containment) | nothing yet — the gate is 0.8.0's; the seam (`packages/platform/observability`) ships its invariant as header contract, with the redaction pass proved by its own suite in the `unit` lane | a vendor telemetry SDK imported outside a declared sink; a sink attached ahead of the redaction pass — no shipped layer scans for either | **Declared in 0.7.0, deferred MACHINE-HELD: this row's Target is the hold, read by `docs-sync` like every other date in this column.** The 0.8.0 gate asserts no vendor telemetry SDK import outside declared sinks and every sink behind the redaction pass — the seam header's own invariant (`packages/platform/observability/src/index.ts`, "NO VENDOR SDK, on purpose"). | `unit` (the redaction pass's behavior only; the containment half has no stand-in until the gate lands) | 0.8.0 |
