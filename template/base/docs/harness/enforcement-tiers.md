...
| `framework-floor` | the scheduled floor-review lane's four register riders (framework floor, cc-floor, support register, `security.txt` lapse) | `apps/web/public/.well-known/security.txt` — the RFC 9116 file's `Expires` lapse | `apps/mobile` | **Declared in 1.0.0.** Single-surface by nature: RFC 9116 is a WEB-ORIGIN file served at `/.well-known/security.txt`, and a mobile app has no such well-known path — its vulnerability-disclosure contact is the store listing's support surface, which no gate reads. The other three riders scan registers, not surfaces. The clockless SHAPE half of the same file rides `security-headers` on every validate; only the calendar lapse rides here, schedule-gated. | — (the mobile surface genuinely needs a different instrument: the store-metadata module's listing fields) | — |
