...
| `version-sync` | store/runtime version lockstep | `apps/mobile` (+ `app.config.ts`, `eas.json`) | `apps/web`; the iOS BUILD TOOLCHAIN floor | **Declared in 0.4.0.** `apps/web` is deliberately EXCLUDED from build-number lockstep (a web deploy has no store version); it is bounded instead by the major-agreement check against `@app/api`, which this gate also performs. So the exclusion is a decision, not a gap. **The toolchain floor IS a gap, added in 0.6.0.** Apple has required uploads to build against **Xcode 26 / iOS 26 SDK or later since 2026-04-28** — a fixed floor, not a moving "current SDK" requirement, and macOS is excluded. `eas.json` pins **no `image` on any profile**, so a stale or too-old toolchain produces no gate output and burns a whole build-and-submit cycle. Only a concrete pinned image name is statically checkable (`-xcode-(\d+)` ≥ the floor); `auto`, `latest`, `sdk-NN` and absent are unverifiable and must NOT read as green. Not discharged here because it needs a real image name from EAS's published list, and seeding a fabricated one would break every consumer's build — strictly worse than the gap. Facts and sources: `design/CONFORMANCE-FACTS.md` §3. | — | 0.7.0 |
