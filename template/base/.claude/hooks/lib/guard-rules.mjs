...
export const WRITE_GLOBAL_CHECKS = [
  ...
  },
  {
    ...
    re: /\bcreateCipher\s*\(|['"](?:aes-\d{3}-ecb|des(?:-ede3?)?(?:-cbc)?|rc4(?:-40)?|md5|sha1|RSA-MD5)['"]|\bAES-\d{3}-ECB\b/i,
    message:
  ...
  },
  {
    ...
    re: /\b(?:key|kek|dek|secret|rootkey|masterkey|privatekey)[a-z0-9_]*\s*[:=][^;\n'"]*['"][A-Fa-f0-9]{64}['"]/i,
    message:
  ...
  },
  {
    ...
    re: /\b(?:key|kek|dek|iv|nonce|salt|token|secret)[a-z0-9_]*\s*[:=][^;\n]*\bMath\.random\s*\(/i,
    message:
