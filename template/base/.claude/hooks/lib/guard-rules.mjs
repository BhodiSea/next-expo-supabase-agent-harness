...
export const WRITE_GLOBAL_CHECKS = [
  ...
  {
    // 0.9.5 (e2ee rails). A broken primitive is not a weaker guarantee, it is
    // none: ECB leaks plaintext structure block by block, createCipher derives a
    // key with a static IV, and MD5/SHA-1/DES/RC4 have no defensible use in new
    // code. Matched in ALGORITHM-ARGUMENT position (a quoted spec string or the
    // legacy constructor) so prose, a variable named `md5sum`, or a migration
    // comment cannot trip it. The lint twin (crypto-primitives-one-door) keeps
    // application code away from the engines entirely; this catches the write.
    id: 'weak-crypto-algorithm',
    // Three shapes, because a developer on THIS stack writes the third: the
    // legacy static-IV constructor, a node-style algorithm string, and the
    // WebCrypto spelling — `{ name: 'HMAC', hash: 'SHA-1' }` — which the first
    // draft of this rule did not know at all, on rails whose own provider is
    // crypto.subtle-based. `sha-1` vs `sha1` is exactly that gap.
    re: /\bcreateCipher\s*\(|['"](?:aes-\d{3}-ecb|des(?:-ede3?)?(?:-ecb|-cbc)?|bf-(?:ecb|cbc)|rc2-(?:ecb|cbc)|rc4(?:-40)?|md5|sha1)['"]\s*(?=[,)\]}])|\b(?:hash|name)\s*:\s*['"](?:SHA-1|MD5)['"]|\.digest\s*\(\s*['"](?:SHA-1|MD5)['"]|['"]AES-\d{3}-ECB['"]/i,
    message:
      'That algorithm has no defensible use in new code: ECB leaks plaintext structure, createCipher() derives a key with a static IV, and MD5/SHA-1/DES/RC4 are broken. The harness ships AES-256-GCM behind @app/crypto with a versioned envelope and mandatory AAD — take primitives from the injected CryptoProvider. SOURCE: .claude/rules/encryption.md',
  },
  {
    // 0.9.5. A 32-byte hex literal assigned into a key-shaped name is key
    // material in the repository — the shape a test fixture takes on its way to
    // becoming a production constant. The `secrets` gate sweeps the tree for
    // this class at validate time; this denies the write that creates it.
    // `[^;\n'"]*` after the assignment operator, not `\s*`: a TYPE ANNOTATION
    // sits between the name and the value (`const rootKey: string = '…'`), and
    // an anchored-to-whitespace pattern reads that as a miss — which is how a
    // key literal in a typed file would have walked past this rule.
    id: 'hardcoded-key-material',
    re: /\b[a-z0-9_]*(?:key|kek|dek|secret|passphrase)[a-z0-9_]*\s*[:=][^;'"`]{0,80}['"`][A-Fa-f0-9]{64}['"`]/i,
    message:
      'That is a 256-bit key literal in source. Keys are GENERATED by the platform CSPRNG and live in the platform keystore (@app/crypto KeystoreAdapter) — never in a file, not even a test fixture, because a fixture key is one copy-paste from a production constant. SOURCE: .claude/rules/encryption.md',
  },
  {
    // 0.9.5. The write-guard twin of no-insecure-random-in-crypto-scope, and it
    // exists because the lint rule cannot see an Edit FRAGMENT or a file outside
    // the linted globs. Keyed on the ASSIGNMENT shape (a key/iv/nonce/salt/token
    // name taking a Math.random expression) rather than on Math.random alone, so
    // ordinary jitter and shuffles are untouched.
    id: 'math-random-key-material',
    re: /\b(?:iv|[a-z0-9_]*(?:key|kek|dek|nonce|salt|token|secret|passphrase)[a-z0-9_]*|[a-z0-9_]*iv(?:_[a-z0-9_]*)?)\s*[:=][^;]{0,120}?\bMath\.random\s*\(/i,
    message:
      'A key, IV, nonce or salt derived from Math.random is not one — Math.random is not a CSPRNG on any engine. Use the injected CryptoProvider.randomBytes, or the platform CSPRNG inside a host adapter (apps/mobile/src/host/large-secure-store.ts says the same thing in its own header). SOURCE: .claude/rules/encryption.md',
  },
  {
